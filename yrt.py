...
# RASA_API_ENDPOINT ='https://f694-197-156-76-21.ngrok.io/webhooks/rest/webhook'
...
async def handle_button_click(update, context):
    ...
    response = r.json()
    text = response[0]['text']
    image_url = response[0].get('image', None)
    video_url = response[0].get('video', [])
    buttons = response[0].get('buttons', [])
    ...
    # if image_url:
    #     await query.message.reply_photo(photo=image_url, caption=text, reply_markup=reply_markup)
    # elif video_url:
    #     await query.message.reply_video(video=video_url, caption=text, reply_markup=reply_markup)
    # else:
    #     await query.edit_message_text(text, reply_markup=reply_markup)
